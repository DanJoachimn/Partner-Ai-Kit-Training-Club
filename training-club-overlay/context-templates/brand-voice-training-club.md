---
type: context
created: YYYY-MM-DD
updated: YYYY-MM-DD
status: stub
generated_by: claude-code
---

# Brand Voice Guide — [CLUB_NAME]

> Filled during kick-off Section B (Voice). This is the constitution the Content subagent reads before EVERY public-facing draft.

## The one-line principle
<!-- If [CLUB_NAME] were a person walking into a room, what's the energy? One sentence. -->

## Who reads our content
<!-- Specific. Where they shop, what they aspire to, what they fear. Not "fitness enthusiasts." -->

## How we sound

### Tone words (3-5 that capture how we sound)
<!-- Example: direct, athletic, irreverent, warm-when-it-counts, no-fluff -->

### Sentence shape preferences
<!-- Short or long? Punchy or flowing? Lists or prose? -->

### How we use humor (if at all)
<!-- Self-deprecating? Dry? None? When does it appear? -->

### How we handle the HYROX brand
<!-- HYROX is a registered trademark. How does this club refer to HYROX races, HYROX events, the HYROX format? Respectful but distinct from corporate-HYROX language. -->

## What we don't sound like

### Banned tropes
<!-- "Crush your goals," "transform your life," "fitness journey," "best version of yourself" — list YOUR banned phrases, with reasons. -->

### Brands whose copy makes us cringe
<!-- 2-3 specific brands. WHY they grate — feeds your AI's negative training. -->

## Three reference brands we'd kill for
<!-- 3 brands whose copy we genuinely admire. WHY each one — what specifically lands. -->

## How we talk about members

### When celebrating a win
<!-- Specific over generic. Names matter. Anchors to their actual achievement. -->

### When inviting someone to come back (retention message)
<!-- Warm, specific, no pressure, makes it easy to say yes — see the `member-checkin-draft` skill for examples. -->

### When responding to a new inquiry
<!-- Match their tone. Hot lead = fast and specific. Curious browser = patient and educational. -->

## How we talk about racing

### Pre-race energy
<!-- The 1-2 weeks before a race — how does the club's content feel? Hyped? Focused? Quiet confidence? -->

### Race-day + post-race
<!-- Live posts? Day-after recaps? Member-by-member callouts? -->

### General race language
<!-- How we describe the race itself. Hard? Fun? Brutal? Beautiful? Specific words we use. -->

## Platform-specific notes

### Instagram
<!-- Caption length sweet spot? Hashtag policy? -->

### Newsletter
<!-- Frequency? Section structure? -->

### SMS / WhatsApp (member messaging)
<!-- Length? Formality? Emojis? -->

### Inquiry responses
<!-- Subject lines? First line? Sign-off? -->
