...
> **This file is read by an AI agent (Claude Code) at install time.** A HYROX Training Club operator has just pasted a prompt asking you to install. They're a gym owner, not a developer. They run programming, retention, marketing, ops — alone or with a small team. They want this working, not configured.

---

## Read this carefully before doing anything

All the hard rules from the Personal kit's INSTALL.md apply here (no jargon, confirm before writing, show don't tell, visible progress, no raw errors, pause for physical actions). On top of that:

**HTC-specific hard rules:**
...
3. **Two-phase install.** This kit layers on top of the Personal kit. First run the Personal install (~15 min), then layer the HTC overlay (~10 min), then the kick-off interview gets HTC-flavored variants (~25 min). Total: ~50 min for the deluxe path.
4. **Surface the value early.** Within 5 minutes the operator should see a concrete HTC use case — *"in 20 minutes you'll be able to ask me to draft a member check-in or build a race-anchored block."*

---

## Stage 0 — Greeting + tone-setting (~1 min)

Open with HTC context. Set expectations.
...
The HTC kit DOES NOT duplicate the Personal install. Instead, it triggers the Personal install first.
...
> "✅ Personal AI foundation installed. Folder created, vault scaffolded, 5 core skills installed, scheduled jobs wired up. Now layering on the HTC stuff."

---

## Stage 2 — Clone the HTC overlay (~30 sec)

```bash
mkdir -p "$HOME/Documents/[AI_NAME]/.htc-overlay"
git clone https://github.com/DanJoachimn/Partner-Ai-Kit-Training-Club.git "$HOME/Documents/[AI_NAME]/.htc-overlay"
```
...
for skill in weekly-retention-review weekly-content-batch block-builder member-checkin-draft; do
  cp -R "$HTC_SKILLS_SRC/$skill" "$SKILLS_DST/$skill"
...
> "✅ Installed 4 HTC-specific skills:
...
> "✅ Added gym-flavored folders. `Members/` will fill up as we talk about your members; `Programming/` holds your blocks + scaling templates; `Events/` tracks races your members are signed up to."
...
```bash
TEMPLATES_SRC="$HOME/Documents/[AI_NAME]/.htc-overlay/htc-overlay/context-templates"
CONTEXT_DST="$HOME/Documents/[AI_NAME]/vault/_context"

mkdir -p "$CONTEXT_DST"

for template in club-about brand-voice-htc ideal-member-profile race-calendar-starter; do
...
> "✅ Seeded HTC context templates. The kick-off interview next will fill them in with your specifics."
...
Read the HTC kick-off additions:

```bash
cat "$HOME/Documents/[AI_NAME]/.htc-overlay/htc-overlay/kick-off-htc-additions.md" 2>/dev/null
```

Then invoke `~/.claude/skills/kick-off/SKILL.md` with the HTC additions layered in.
...
If start now → invoke kick-off with HTC additions.

If break → flag pending:

```bash
touch "$HOME/Documents/[AI_NAME]/.kick-off-pending-htc"
```
...
| `[CLUB_NAME]` | Set during HTC kick-off Section A — leave as placeholder until then |
...
Same pattern as Personal INSTALL.md — every failure is recoverable from snapshot. The HTC overlay specifically:

- If `git clone` of the HTC overlay fails (Personal install succeeded) → user has a working Personal AI; HTC overlay can be retried later. Tell them: *"The Personal kit installed clean. The HTC overlay had a hiccup — your AI still works, just without the HTC-specific stuff. Tell me 'install HTC overlay' when you want to try again."*
- If any Coach subagent fails to install → install the rest, log the failed one, tell user *"3 of 4 coaches installed. The Marketing Coach hit an issue — your AI still works, just without that specialized helper. We can retry it later."*

Half-installed is acceptable as long as it's announced and recoverable. Silent failure is not.

---

*This file is the live HTC install playbook. Update it when the install flow changes; the next operator's install will get the new version automatically.*
